...
class Valval {
      ...
      if (typeof item === 'object') {
        ...
        form.addEventListener('submit', event => {
            ...
            if (!this.invalidSize) {
              ...
              item.required && this.checkRequired(item);
  ...
  checkValid(item) {
    this.switchingClasses(item.element, item.classInvalid, item.classValid, item.valid);
  }
  ...
  getDefaultParams(options) {
      for (let item in options) {
        if (item !== 'preventDefault') {
          ...
          // Everything except the date, mail repeat password, button and form
  ...
  start(options) {
    if (typeof options === 'object') {
      ...
      for (let item in options) {
        if (typeof options[item] === 'object' && !options[item].formSubmit && !options[item].button) {
        ...
        this.submitForm(options[item], options);
      }
    }
